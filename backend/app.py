from flask import Flask, jsonify, request
from datetime import datetime
import random
import os
from dotenv import load_dotenv
from flask_cors import CORS
from birth_chart_service import get_sun_sign, get_moon_sign, get_rising_sign, get_planetary_positions
from tarot_service import shuffle_deck, draw_cards, get_card_meaning

load_dotenv()

app = Flask(__name__)
CORS(app)  # Enable CORS for all routes

# Sample horoscope data
HOROSCOPES = {
    "aries": "今天是新的開始，勇敢地追逐你的夢想吧！",
    "taurus": "穩定的一天，適合專注於長期目標和財務規劃。",
    "gemini": "沟通順暢，適合進行社交活動和學習新知識。",
    "cancer": "關注家庭和情感需求，今天適合好好照顧自己。",
    "leo": "你的魅力與創造力達到峰值，是展現自己的好時機。",
    "virgo": "注重細節的一天，工作和學習都會有不錯的進展。",
    "libra": "人際關係和諧，適合處理合作事項和尋求平衡。",
    "scorpio": "深度思考和內省的一天，適合探索內心世界。",
    "sagittarius": "冒險精神高漲，適合計畫旅行或嘗試新事物。",
    "capricorn": "踏實努力會有回報，專注於職涯發展和目標達成。",
    "aquarius": "獨特想法與創意爆發，適合進行創新專案。",
    "pisces": "直覺特別敏銳，適合藝術創作和精神修練。\n"
}

# Weekly horoscope data (static for simplicity)
WEEKLY_HOROSCOPES = {
    "aries": "這週是新機會的週期，勇敢地接受挑戰，你的領導能力將被認可。",
    "taurus": "這週財務方面穩定，適合進行長期投資規劃，人際關係和諧。",
    "gemini": "這週沟通順暢，適合學習新技能和進行社交活動，創意靈感爆發。",
    "cancer": "這週關注家庭和情感需求，適合進行家庭活動和自我照顧，直覺特別敏銳。",
    "leo": "這週你的魅力與創造力達到峰值，是展現自己和進行創意專案的好時機。",
    "virgo": "這週注重細節的一天，工作和學習都會有不錯的進展，適合整理和規劃。",
    "libra": "這週人際關係和諧，適合處理合作事項和尋求平衡，適合進行團隊項目。",
    "scorpio": "這週深度思考和內省的一天，適合探索內心世界和進行精神修練。",
    "sagittarius": "這週冒險精神高漲，適合計畫旅行或嘗試新事物，適合戶外活動和探索。",
    "capricorn": "這週踏實努力會有回報，專注於職涯發展和目標達成，適合進行職涯規劃。",
    "aquarius": "這週獨特想法與創意爆發，適合進行創新專案和科技探索，適合團隊合作。",
    "pisces": "這週直覺特別敏銳，適合藝術創作和精神修練，適合進行藝術和音樂創作。\n"
}

# Monthly horoscope data (static for simplicity)
MONTHLY_HOROSCOPES = {
    "aries": "本月是新機會的月份，適合開始新專案和設定目標。",
    "taurus": "本月財務穩定，適合進行投資和儲蓄規劃。",
    "gemini": "本月沟通順暢，適合學習新技能和進行社交活動，創意靈感爆發。",
    "cancer": "本月關注家庭和情感需求，適合進行家庭活動和自我照顧，直覺特別敏銳。",
    "leo": "本月你的魅力與創造力達到峰值，是展現自己和進行創意專案的好時機。",
    "virgo": "本月注重細節的一天，工作和學習都會有不錯的進展，適合整理和規劃。",
    "libra": "本月人際關係和諧，適合處理合作事項和尋求平衡，適合進行團隊項目。",
    "scorpio": "本月深度思考和內省的一天，適合探索內心世界和進行精神修練。",
    "sagittarius": "本月冒險精神高漲，適合計畫旅行或嘗試新事物，適合戶外活動和探索。",
    "capricorn": "本月踏實努力會有回報，專注於職涯發展和目標達成，適合進行職涯規劃。",
    "aquarius": "本月獨特想法與創意爆發，適合進行創新專案和科技探索，適合團隊合作。",
    "pisces": "本月直覺特別敏銳，適合藝術創作和精神修練，適合進行藝術和音樂創作。\n"
}

@app.route('/')
def index():
    return jsonify({
        "message": "Stellar Whisper API",
        "version": "1.0.0",
<<<<<<< HEAD
        "endpoints": ["/horoscope/<sign>", "/horoscope/today", "/horoscope/weekly/<sign>", "/horoscope/<sign>/<date>", "/horoscope/monthly/<sign>"]
        "endpoints": ["/horoscope/<sign>", "/horoscope/today", "/horoscope/weekly/<sign>", "/horoscope/<sign>/<date>"]
=======
        "endpoints": ["/horoscope/<sign>", "/horoscope/today", "/horoscope/weekly/<sign>", "/horoscope/<sign>/<date>", "/tarot/deck", "/tarot/draw/single", "/tarot/draw/three-card"]
>>>>>>> feat/add-tarot-card-reading
    })

@ app.route('/horoscope/<sign>')
def get_horoscope(sign):
    sign = sign.lower()
    if sign in HOROSCOPES:
        return jsonify({
            "sign": sign,
            "horoscope": HOROSCOPES[sign],
            "date": datetime.now().strftime("%Y-%m-%d")
        })
    else:
        return jsonify({"error": "Invalid zodiac sign"}), 400

@app.route('/horoscope/today')
def get_today_horoscopes():
    today = datetime.now().strftime("%Y-%m-%d")
    horoscopes = []
    for sign, text in HOROSCOPES.items():
        horoscopes.append({
            "sign": sign,
            "horoscope": text,
            "date": today
        })
    return jsonify({
        "date": today,
        "horoscopes": horoscopes
    })

@app.route('/api/birth-chart', methods=['POST'])
def get_birth_chart():
    """
    Calculate birth chart based on birth date, time, and location.
    
    Expected JSON payload:
    {
        "birth_date": "YYYY-MM-DD",
        "birth_time": "HH:MM:SS",
        "latitude": float,
        "longitude": float,
        "timezone": "UTC offset or timezone string"
    }
    """
    try:
        data = request.get_json()
        
        # Validate required fields
        required_fields = ['birth_date', 'birth_time', 'latitude', 'longitude']
        for field in required_fields:
            if field not in data:
                return jsonify({"error": f"Missing required field: {field}"}), 400
        
        # Parse date and time
        birth_date = datetime.strptime(data['birth_date'], "%Y-%m-%d")
        birth_time = datetime.strptime(data['birth_time'], "%H:%M:%S")
        
        # Combine date and time for calculation
        birth_datetime = datetime(
            birth_date.year, birth_date.month, birth_date.day,
            birth_time.hour, birth_time.minute, birth_time.second
        )
        
        # Get coordinates
        latitude = float(data['latitude'])
        longitude = float(data['longitude'])
        
        # Calculate birth chart components
        sun_sign = get_sun_sign(birth_datetime)
        moon_sign = get_moon_sign(birth_date, birth_time, latitude, longitude)
        rising_sign = get_rising_sign(birth_date, birth_time, latitude, longitude)
        planetary_positions = get_planetary_positions(birth_date, birth_time, latitude, longitude)
        
        # Prepare response
        result = {
            "birth_date": data['birth_date'],
            "birth_time": data['birth_time'],
            "latitude": latitude,
            "longitude": longitude,
            "sun_sign": sun_sign,
            "moon_sign": moon_sign,
            "rising_sign": rising_sign,
            "planetary_positions": planetary_positions,
            "calculated_at": datetime.now().isoformat()
        }
        
        return jsonify(result)
        
    except ValueError as e:
        return jsonify({"error": f"Invalid input format: {str(e)}"}), 400
    except Exception as e:
        return jsonify({"error": f"Calculation failed: {str(e)}"}), 500

@app.route('/tarot/deck', methods=['GET'])
def get_tarot_deck():
    """
    Returns a shuffled tarot deck (without reversed status, as that is determined at draw time).
    """
    deck = shuffle_deck()
    # Remove the 'reversed' key if it exists (it shouldn't in the base deck, but just in case)
    for card in deck:
        card.pop('reversed', None)
    return jsonify({
        "deck": deck,
        "count": len(deck)
    })

@app.route('/tarot/draw/single', methods=['POST'])
def draw_single_card():
    """
    Draws a single tarot card.
    """
    try:
        # Get a shuffled deck and draw one card
        deck = shuffle_deck()
        drawn_cards = draw_cards(num_cards=1, deck=deck)
        card = drawn_cards[0]
        
        # Get the meaning based on orientation
        meaning = get_card_meaning(card)
        
        return jsonify({
            "card": card,
            "meaning": meaning
        })
    except Exception as e:
        return jsonify({"error": f"Failed to draw card: {str(e)}"}), 500

@app.route('/tarot/draw/three-card', methods=['POST'])
def draw_three_card_spread():
    """
    Draws a three-card tarot spread (past, present, future).
    """
    try:
        # Get a shuffled deck and draw three cards
        deck = shuffle_deck()
        drawn_cards = draw_cards(num_cards=3, deck=deck)
        
        # Get meanings for each card
        cards_with_meaning = []
        for i, card in enumerate(drawn_cards):
            meaning = get_card_meaning(card)
            cards_with_meaning.append({
                "position": ["past", "present", "future"][i],
                "card": card,
                "meaning": meaning
            })
        
        return jsonify({
            "spread": cards_with_meaning
        })
    except Exception as e:
        return jsonify({"error": f"Failed to draw spread: {str(e)}"}), 500

@app.route('/horoscope/weekly/<sign>')
def get_weekly_horoscope(sign):
    sign = sign.lower()
    if sign in WEEKLY_HOROSCOPES:
        return jsonify({
            "sign": sign,
            "horoscope": WEEKLY_HOROSCOPES[sign],
            "date": datetime.now().strftime("%Y-%m-%d")
        })
    else:
        return jsonify({"error": "Invalid zodiac sign"}), 400

@app.route('/horoscope/monthly/<sign>')
def get_monthly_horoscope(sign):
    sign = sign.lower()
    if sign in MONTHLY_HOROSCOPES:
        return jsonify({
            "sign": sign,
            "horoscope": MONTHLY_HOROSCOPES[sign],
            "date": datetime.now().strftime("%Y-%m-%d")
        })
    else:
        return jsonify({"error": "Invalid zodiac sign"}), 400

@app.route('/horoscope/<sign>/<date>')
def get_horoscope_specific_date(sign, date):
    sign = sign.lower()
    # Validate the sign
    if sign not in HOROSCOPES:
        return jsonify({"error": "Invalid zodiac sign"}), 400
    # Validate the date format (YYYY-MM-DD)
    try:
        datetime.strptime(date, '%Y-%m-%d')
    except ValueError:
        return jsonify({"error": "Invalid date format. Use YYYY-MM-DD."}), 400
    return jsonify({
        "sign": sign,
        "horoscope": HOROSCOPES[sign],
        "date": date
    })

if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5000, debug=True)